...
class WeightedECDF(StepFunction):
    ...
    def inverse(self, pp):
        ...
        if 0.5 in pp and np.all(np.array(self.weights) == self.weights[0]):
            results[pp.index(0.5)] = np.median(self.values)
        ...
        if 3 in pp:
            ...
            results[pp.index(3)] = np.sqrt(
                np.sum(self.weights * (self.values - mu) ** 2) / np.sum(self.weights)
            )
